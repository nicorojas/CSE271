...
/**
 * AdminStaff is responsible for administrative work such as enrolling
 * students in a class, assigning a LectureHall to a class, and 
...
public class AdminStaff extends LectureHall{
	...
	public void enroll(Student a, Class b){
		...
		if(b.getCurrentAmount() >= (b.getHall()).getCapacity()){
			// Empty 'if statement' because if the # of students currently enrolled exceeds the hall
			// maximum capacity, then the student is not added to the class.
		}
		
		else{
			b.setCurrentAmount(); // +1 to current # of students enrolled in class
			a.setClasses(b);
		}
	}
